...
__device__ inline bool intersect(const Sphere* spheres, const Ray &r, double &t, int &id){
  double n=sizeof(spheres)/sizeof(Sphere), d, inf=t=1e20;
  for(int i=int(n);i--;) if((d=spheres[i].intersect(r))&&d<t){t=d;id=i;}
  return t<inf;
}
__device__ Vec radiance(const Sphere* spheres, const Ray &r, int depth, unsigned short *Xi, curandState* state){
  double t;                               // distance to intersection
  int id=0;                               // id of intersected object
  if (!intersect(spheres, r, t, id)) return Vec(); // if miss, return black
  ...
  if (obj.refl == DIFF){                  // Ideal DIFFUSE reflection
    ...
    return obj.e + f.mult(radiance(spheres,Ray(x,d),depth,Xi,state));
  } else if (obj.refl == SPEC)            // Ideal SPECULAR reflection
    return obj.e + f.mult(radiance(spheres,Ray(x,r.d-n*2*n.dot(r.d)),depth,Xi,state));
  Ray reflRay(x, r.d-n*2*n.dot(r.d));     // Ideal dielectric REFRACTION
  bool into = n.dot(nl)>0;                // Ray from outside going in?
  double nc=1, nt=1.5, nnt=into?nc/nt:nt/nc, ddn=r.d.dot(nl), cos2t;
  if ((cos2t=1-nnt*nnt*(1-ddn*ddn))<0)    // Total internal reflection
    return obj.e + f.mult(radiance(spheres,reflRay,depth,Xi,state));
  Vec tdir = (r.d*nnt - n*((into?1:-1)*(ddn*nnt+sqrt(cos2t)))).norm();
  double a=nt-nc, b=nt+nc, R0=a*a/(b*b), c = 1-(into?-ddn:tdir.dot(n));
  double Re=R0+(1-R0)*c*c*c*c*c,Tr=1-Re,P=.25+.5*Re,RP=Re/P,TP=Tr/(1-P);
  return obj.e + f.mult(depth>2 ? (curand_uniform_double(state)<P ?   // Russian roulette
    radiance(spheres,reflRay,depth,Xi,state)*RP:radiance(spheres,Ray(x,tdir),depth,Xi,state)*TP) :
    radiance(spheres,reflRay,depth,Xi,state)*Re+radiance(spheres,Ray(x,tdir),depth,Xi,state)*Tr);
}

__global__ void render(Sphere* spheres, Vec* c, int samps){
    ...
    for (int sy=0, i=(h-y-1)*w+x; sy<2; sy++) 
        for (int sx=0; sx<2; sx++, r=Vec()){        // 2x2 subpixel cols
          for (int s=0; s<samps; s++){
            ...
            r = r + radiance(spheres, Ray(cam.o+d*140,d.norm()),0,Xi,&state)*(1./samps);
...
int main(int argc, char *argv[]){
  ...
  Vec *h_c=new Vec[w*h];
  Vec *c;
  cudaMalloc((void **)&c, sizeof(h_c));
  cudaMalloc((void **)&spheres, sizeof(h_spheres));
  cudaMemcpy(c, h_c, sizeof(Vec), cudaMemcpyHostToDevice);
  cudaMemcpyToSymbol(spheres, &h_spheres, sizeof(h_spheres), 0, cudaMemcpyHostToDevice);

  dim3 dimGrid((w-1)/64+1, (h-1)/64+1, 1);
  dim3 dimBlock(64, 64, 1);
  render<<<dimGrid, dimBlock>>>(spheres, c, samps);

  cudaMemcpyFromSymbol(h_c, c, sizeof(Vec), 0, cudaMemcpyDeviceToHost);
